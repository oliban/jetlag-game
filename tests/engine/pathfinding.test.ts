...
describe('Pathfinding (Dijkstra)', () => {
  it('finds path between adjacent stations', () => {
    const result = findShortestPath('paris', 'lille-europe');
    expect(result).not.toBeNull();
    expect(result!.path).toEqual(['paris', 'lille-europe']);
    expect(result!.distance).toBe(225);
  });

  it('finds path between non-adjacent stations', () => {
    const result = findShortestPath('london', 'berlin-hbf');
    expect(result).not.toBeNull();
    expect(result!.path.length).toBeGreaterThan(2);
    expect(result!.path[0]).toBe('london');
  ...
  it('returns same station path with distance 0 for same start/end', () => {
    const result = findShortestPath('paris', 'paris');
    expect(result).not.toBeNull();
  ...
  it('returns null for non-existent stations', () => {
    const result = findShortestPath('nonexistent', 'paris');
  ...
  it('finds shortest path (not just any path)', () => {
    ...
    const result = findShortestPath('paris', 'lyon-part-dieu');
    expect(result).not.toBeNull();
    // Direct connection is 465km
    expect(result!.distance).toBe(465);
    expect(result!.path).toEqual(['paris', 'lyon-part-dieu']);
  });
